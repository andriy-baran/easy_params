  ...
  describe '#coerce' do
    context 'when initial type setup' do
      it 'tries to coerce to integer' do
        ...
        expect(type.coerce('1.1')).to eq '1.1'
        expect(type.coerce('1.1'.to_d)).to eq '0.11e1'
        expect(type.coerce('1')).to eq '1'
        expect(type.coerce('2011-11-03')).to eq '2011-11-03'
        expect(type.coerce('33-03-2011')).to eq '33-03-2011'
        expect(type.coerce('2011-11-03 10:23:45')).to eq '2011-11-03 10:23:45'
        expect(type.coerce('2011-11-03 30:23:45')).to eq '2011-11-03 30:23:45'
        expect(type.coerce({})).to eq '{}'
        expect(type.coerce([])).to eq '[]'
      end
    end
    ...
    context 'when default set' do
      it 'tries to coerce to integer' do
        ...
        expect(type_with_default.coerce('')).to eq ''
        expect(type_with_default.coerce(1)).to eq '1'
        expect(type_with_default.coerce(1.1)).to eq '1.1'
        expect(type_with_default.coerce('1.1')).to eq '1.1'
        expect(type_with_default.coerce('1.1'.to_d)).to eq '0.11e1'
        expect(type_with_default.coerce('1')).to eq '1'
        expect(type_with_default.coerce('2011-11-03')).to eq '2011-11-03'
        expect(type_with_default.coerce('33-03-2011')).to eq '33-03-2011'
        expect(type_with_default.coerce('2011-11-03 10:23:45')).to eq '2011-11-03 10:23:45'
        expect(type_with_default.coerce('2011-11-03 30:23:45')).to eq '2011-11-03 30:23:45'
        expect(type_with_default.coerce({})).to eq '{}'
        expect(type_with_default.coerce([])).to eq '[]'
      end
    end
    ...
    context 'when normalizer set' do
      it 'is called before coercion' do
        value = double(to_norm: 2011)
        type_with_normalizer = type.normalize { |v| v.to_norm }
        expect(type_with_normalizer.coerce(value)).to eq '2011'
      end
    end
    ...
    context 'when default set and normalizer set' do
      it 'is normalized and coerced' do
        ...
        expect(configured_type.coerce(value)).to eq ''
