...
RSpec.describe EasyParams::Types::Time do
  ...
  describe '#coerce' do
    context 'when initial type setup' do
      it 'tries to coerce to integer' do
        ...
        expect(type.coerce('1')).to eq nil
        expect(type.coerce('2011-11-03')).to eq Time.parse('2011-11-03')
        expect(type.coerce('33-03-2011')).to eq nil
        expect(type.coerce('2011-11-03 10:23:45')).to eq Time.parse('2011-11-03 10:23:45')
    ...
    context 'when default set' do
      it 'tries to coerce to integer' do
        ...
        expect(type_with_default.coerce('1')).to eq 10
        expect(type_with_default.coerce('2011-11-03')).to eq Time.parse('2011-11-03')
        expect(type_with_default.coerce('33-03-2011')).to eq 10
        expect(type_with_default.coerce('2011-11-03 10:23:45')).to eq Time.parse('2011-11-03 10:23:45')
    ...
    context 'when normalizer set' do
      it 'is called before coercion' do
        ...
        expect(type_with_normalizer.coerce(value)).to eq Time.parse('2011-11-03')
      end
    end
